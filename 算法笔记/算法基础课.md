...
> 三重循环时为什么不是`f[i][j] = max(f[i-1][j],f[i-1][j-v[i]*k]+w[i]*k)`的回答：
> 
> 你这么写的想法是考虑第i个物品不取的情况吧，但是看三层循环的版本`f[i][j] = max(f[i][j],f[i-1][j-kv[i]]+k*w[i])`;可以发现，`k==0`的时候就是不取第i个的情况，然后用这个值去不断更新`f[i][j]`。如果按你的写法，并不会每次把`f[i][j]`往大了更新，而是不确定的
...
**那为什么完全背包不会有最后一项？**
完全背包由于对每种物品**没有选择个数的限制**，所以只要体积够用就可以一直选，没有最后一项。
#### 经典二进制优化
如果直接三重循环，如果直接遍历转化为01背包问题，是每次都拿一个来问，取了好还是不取好。那么根据数据范围，这样的时间复杂度是$O(n^3)$,当N,V,S均很大时会爆TLE,。故此处需要采用**二进制优化**
根据二进制计算原理，任何一个十进制整数$X$均可以用1，2，4，8等2的倍数相加得到，这些2的倍数只有$log_2 X$个。题目中第$i$个物品有$s_i$个，用$log_2 s_i$就能组合出 $0 \sim s_i$种情况

注意拆分的具体实现，**不能全部拆成2的倍数**，而是先按2的倍数从小到大拆，最后是一个小于或等于最大倍数的余数。从而保证拆出的数相加在$[1,s_i]$范围内，不会大于$s_i$。这样就转换成了一个**01背包问题**，最后从后往前更新

最后时间复杂度可以从$NVS$优化为$NVlogS$
```c++
#include<bits/stdc++.h>
using namespace std;
...
int main(void){
	...
	n = cnt;
	// 最后进行01背包，需要从后往前更新
	// 这里因为是组别，最后转化成01背包了以后就变成了选这组还是不选这组的问题了，所以能够表示1~s[i]的所有数
	for(int i = 1; i <= n; i++)
		for(int j = m; j >= v[i]; j--)
			// 从后往前更新
			f[j] = max(f[j], f[j-v[i]]+w[i]);
...
$N$组，每组若干个，**每组**最多**选一个**
- 状态表示
	- 集合 只从前$i$组物品中轩，且总体积不大于$j$的所有选法
	- 属性
- 状态计算
	- 集合划分
	- 枚举第 $i$ 组物品选哪个/不选
	- 第 $i$ 组物品一个都不选：$f[i-1, j]$
	- 第 $i$ 组物品选第 $k$ 个：$f[i-1, j-v[i,k]] + w[i,k]$
```c++
#include<bits/stdc++.h>
...
int main(void){
	// 同样可优化为1维
	// 如果转移时候用的是上一层，就需要从大到小枚举体积
	// 如果用的是本层的，就需要从小到大进行枚举
	cin >> n >> m;
...
```
先标号：左边行号，右边列号
![image.png|380](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240409211700.png)
...
- 状态计算：集合划分的过程
	- $f[i][j]$：来自左边和来自右边
	- 来自左边：$f[i-1, j-1] + a[i,j]$
	- 来自右边：$f[i-1,j] + a[i,j]$
	- 最后取max
- 时间复杂度判断：状态数量 $\times$ 转移计算量
```c++
#include<bits/stdc++.h>
...
int main(void){
	...
	for(int i = 1; i <= n; i++)
	 ...
	 // 注意这里要从0开始出发
	for(int i = 0; i <= n; i++)
		// 这里是因为边界，由于右边的边界部分也是有两边过来，所以这里需要给边缘部分初始化-INF来控制边界
		for(int j = 0; j<= i + 1; j++)
			f[i][j] = -INF;

	f[1][1] = a[1][1];
	// 这里f[1][1]因为是三角形顶端，可以省略
	for(int i = 2; i <= n; i++)
		for(int j = 1; j <= i; j++)
...
```
> 关于数组下标设置问题：
> 当状态转移方程涉及$i-1$时，通常数组从0开始，这样可以减少边界判断
...
- 状态表示：$f[i]$表示从第一个数字开始算，以$a[i]$结尾的最大的上升序列的长度
	- 集合：所有以第$i$个数结尾的**上升子序列**的集合
	- 属性：集合中每一个上升子序列的长度的最大值
	- 最后答案只需要从$f[1]$到$f[n]$遍历，即可找到从第1个到最后一个数为结尾的上升子序列的**最长值**
- 状态计算：集合划分
	- 为0：以第0个数结尾，为1：以第1个数结尾...一直到i-1
![image.png|473](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240410143333.png)
	- 如果上一个数是$a[j]$，且满足$a[j] < a[i]$，那么对于$f[i]$最长上升子序列长度应该为$f[j] + 1$
	- $f[i]=\max (f[j] + 1), a[j] < a[i]$
- 时间复杂度：
	- 状态数量 $n$ $\times$ 状态转移次数 $n$ = $O(n^2)$
```c++
#include<bits/stdc++.h>
...
int main(void){
	...
	for(int i = 1; i <= n; i++){
		f[i] = 1; // 只有 a[i] 一个数的情况，找不到前面数字小于自己的时候就为1
		g[i] = 0;
		for(int j = 1; j < i; j++){  // 枚举所有 i 之前的元素
			if(a[j] < a[i]){
				if(f[i] < f[j] + 1){
					f[i] = f[j] + 1;
					g[i] = j; // 用 g 数组记录 f 是从哪里转移的
				}
			}
		}
	}
...
```
#### 如何优化？
 ![image.png](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240423190255.png)
以前两个数为例，发现如果$a_i>3$，那么也一定有$a_i>1$，但是反过来不成立。所以$1$比$3$要好，因为1更小，可以在后面接更多的数

因此可以按照长度分类，每一个长度均存下该长度下最小的$a[i]$值
![image.png](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240423191102.png)
由该规律可看出，长度越长，最后值一定越大；即结尾的最小值单调递增

如果想求以$a[i]$结尾的最长上升子序列长度：可以将其接到最大的小于$a[i]$的数的后面(假设为$q[j]$)，这样对应的位置就是它的长度$j+1$，然后可以更新序列值$q[j+1]$，发现可以用二分查找$a[i]$对应的位置
...
- 状态表示 $f[i,j]$ （一般有两个序列都用二维数组）
	- 集合 所有在第一个序列的**前 i 个字母**出现，且在第二个序列的**前 j 个字母**中出现的公共子序列（惯例）
	- 属性 max
- 状态计算
	- 集合划分是难点
	- 以 $a[i]，b[j]$ 是否包含在子序列中进行划分
	- 00 两个都不选，10选$a[i]$，01选$b[j]$，11两个都选
	- 00 $f[i-1,j-1]$，这个一定会被包含在 $f[i-1,j]$或$f[i,j-1]$里面
	- 11 $f[i-1, j-1]+1$，只有当 $a[i] = b[j]$ 的时候出现
	- 01 不能表示为$f[i-1,j]$，因为这里表示的是所有在第一个序列的**前 i -1个字母**出现，且在第二个序列的**前 j 个字母**中出现的公共子序列，但没有说明 $b[j]$ 一定被选中了；对于10同理
	- 但是 $f[i-1,j]$一定包含了01这种情况
	- 所以取 max 可以得到
	- 求 max 可以重复
	![image.png](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240410152652.png)

	eg. 求 A,B,C的最大值，可以重复，只要不漏就行
	![image.png|500](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240410152327.png)
```c++
#include<bits/stdc++.h>
...
每次只能合并**相邻的两堆**，合并的代价为这两堆石子的质量之和，合并后与这两堆石子相邻的石子将和新堆相邻，合并时由于选择的顺序不同，合并的总代价也不相同。
...
- 状态表示 $f[i,j]$ 
	- 集合 表示所有将第 i 堆石子到第 j 堆石子合并成一堆石子的合并方法
	- 属性 min
- 状态计算
	- 以最后一次分界线的位置来分类
	- 对于$f[i][j]$，划分成$f[i,k]$和$f[k,j]$
	- 这样代价就变为$f[i,k] + f[k,j] + i\sim k石子总重量$
	- 对于石子重量，可以用前缀和来存储
	- 于是状态转移方程变为$f[i,j] = \min\{f[i,k] + f[k,j]+s[j] -s[i-1]\}$
	- ![image.png](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240410160329.png)
```c++
#include<bits/stdc++.h>
using namespace std;
...
计算a~b中x出现的次数：与前缀和思想类似
`count(b,x) - count(a-1,x)`

分类讨论！

![image.png](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240410164656.png)
边界：
1. 1出现在最高位，第一种情况不存在
2. 枚举数字0出现在第4位，前3位不能为0，因为这样会变成前导0，但是不允许这样的情况存在
...
![image.png|391](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240416192835.png)

#### 题解
每次选择两个权值最小的点进行合并

证明：
- 最小的两个数一定是树中最深的点，并且可以互为兄弟
*反证法：* 假设不是最深的
假设$f$和$a$是最小的两个点，但它们却不在同一层，那么可以把$f$和$b$交换，得到的结果一定更小$$2f + 3b > 3f + 2b$$
- $n$的最优解一定是$n-1$的最优解
*证明：* 第一步合并能得到最优解，那么转为考虑$f(n-1)$的最优解：![image.png|326](https://cdn.jsdelivr.net/gh/BlackJack0083/image@main/img/20240416194952.png)
$f(n) = f(n-1) + a + b$
由于每种编码都会加上$a+b$，故最后变成了$n-1$求最小问题，变成小一级的哈夫曼问题
```c++
#include<bits/stdc++.h>
